...
const migrations: Migration[] = [
  {
    ...
    up: (db) => {
      ...
      db.run(`CREATE UNIQUE INDEX idx_mem_dedup ON memories(scope, scope_key, content_hash) WHERE deleted_at IS NULL`);
      db.run(`CREATE VIRTUAL TABLE memories_fts USING fts5(id UNINDEXED, body, tokenize='unicode61')`);
      db.run(`
  ...
  },
  {
    ...
    up: (db) => {
      ...
      `);
      db.run(`CREATE VIRTUAL TABLE session_summaries_fts USING fts5(id UNINDEXED, body, tokenize='unicode61')`);
    },
  },
];
