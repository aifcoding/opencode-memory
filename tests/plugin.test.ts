...
test("pin 配额超额拒绝", async () => {
  const r = await hooks.tool.memory_store.execute({ title: "长", content: "x".repeat(5000), type: "fact", tags: [] }, ctx);
...
test("session.compacted 自动归档 + 幂等 + 非 compaction 忽略", async () => {
  const fakeMessages = [
    { info: { id: "msg_comp_1", agent: "compaction" }, parts: [{ type: "text", text: "头像加载失败的排查结论" }] },
    { info: { id: "msg_other", agent: "orchestrator" }, parts: [{ type: "text", text: "普通消息" }] },
  ];
