...
test('英文标识符保留（camelCase 拆分 + 下划线）', () => {
  const e = store.create({
    ...
    trust: 'high',
    title: 't',
    content: 'use SqliteMemoryStore and foo_bar here',
  });
...
test('scope 过滤 + 软删排除', () => {
  ...
  expect(
    store.search('memory', { scope: { scope: 'project', scopeKey: '/a' } }).map((h) => h.id),
  ).toEqual([a.id]);
  store.softDelete(a.id);
  expect(store.search('memory', { scope: { scope: 'project', scopeKey: '/a' } })).toEqual([]);
  expect(
    store.search('memory', { scope: { scope: 'project', scopeKey: '/b' } }).map((h) => h.id),
  ).toEqual([b.id]);
});
...
test('结果带 id + score 且降序', () => {
  ...
  store.create({
    ...
    trust: 'high',
    title: 't',
    content: 'memory memory memory system',
  });
