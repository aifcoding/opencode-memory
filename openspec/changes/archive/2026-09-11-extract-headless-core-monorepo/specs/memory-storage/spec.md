...
#### Scenario: CRUD 非法输入与元数据更新

- **WHEN** 创建记忆时使用非法 scope/origin/trust 或更新合法记忆的标题、摘要、类型、标签
- **THEN** 非法值被存储约束拒绝，合法元数据更新不清除 embedding 且同步 FTS
...
#### Scenario: KV upsert/list/delete

- **WHEN** 调用者以同一 contextKey 和 key 重复设置、列出或删除值
- **THEN** 后一次设置覆盖前一次值，list 返回当前值，delete 删除该值并对不存在的 key 返回 not_found

#### Scenario: upsert 与清理

- **WHEN** kv_set 同 key 重复写入
- **THEN** 覆盖旧值；kv_del 删除；按 contextKey 列出全部键值
