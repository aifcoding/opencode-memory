...
条目 SHALL 按 `(scope, scope_key)` 隔离查询，查询缺省不跨 scope。
...
条目删除 SHALL 为软删（置 deleted_at），可恢复；`(scope, scope_key, content_hash)` 在未删除状态下 SHALL 唯一。

#### Scenario: 软删可恢复
- **WHEN** 删除条目
- **THEN** 置 deleted_at 而非物理删除，检索默认排除；恢复可清除 deleted_at
