...
Core SHALL NOT 返回中文或 OpenCode 专属文案。Pin 使用判别联合（pinned/quota_exceeded/summary_required/not_found/deleted）；归档返回 inserted/duplicate。OpenCode Adapter 负责格式化为工具输出。
...
高层入口：`storeMemory`、`recallMemories`、`listMemories`、`readMemory`、`forgetMemory`、`pinMemory`、`unpinMemory`、`listPinnedMemories`、`getPinnedContext`、`archiveSummary`、`recallSummaries`、`setContextValue`、`getContextValue`、`listContextValues`、`deleteContextValue`、`close`。

方法使用对象参数和结构化返回值。

## Risks / Trade-offs

- 公开 API 过早固化 → 仅暴露真实业务操作，内部 SQL、迁移和 tokenizer 不公开。
